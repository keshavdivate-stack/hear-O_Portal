...
/* ---------------- Clinical: Medications ----------------
   Each day is one of three states: taken (true), missed (false, the patient
   was due a dose and it wasn't logged as taken), or not recorded (null, no
   data came in for that day at all -- e.g. a sync gap -- distinct from a
   confirmed miss). */
...
document.querySelectorAll(".compliance-highlight").forEach((box) => {
  ...
  box.classList.add(sufficient ? "compliance-highlight-blue" : "compliance-highlight-red");
  box.querySelector(".goal-progress-fill")?.classList.add(sufficient ? "fill-blue" : "fill-red");
});
...
/* The edge tab (and its accent line) stays visible and slides along with
   the panel instead of disappearing -- see ".chat-panel.open ~ .chat-tab"
   in style.css, which shifts it left by the panel's width so it lands
   pinned to the panel's own top-left corner once open. */
