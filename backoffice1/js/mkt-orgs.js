...
/* ---------------- Organization Type filter (Commercial / Study / R&D) ----------------
   Single-select, reuses the same .mkt-org-ms markup/styling as the org
   multiselect below for visual consistency. Defaults to "Commercial". */
const MKT_ORG_TYPES = [
...
function mktRenderOrgSelect(elId, onChange) {
  ...
  function visibleOrgs() {
    return MKT_ORG_LIST.filter((o) => o.id === "all" || o.type === typeFilter);
  }
