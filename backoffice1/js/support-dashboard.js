...
const urgentCount = supDashTickets.filter((t) => t.severity === "Critical").length;
...
const supDashStats = [
  ...
  { num: urgentCount, label: "Critical Severity", color: "var(--red)", icon: `<path d="M12 9v4"/><path d="M12 17h.01"/><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.73 3h16.9a2 2 0 0 0 1.73-3L13.71 3.86a2 2 0 0 0-3.42 0Z"/>` },
...
const supDashCritical = supDashTickets
  .filter((t) => t.severity === "Critical" || t.status === "Escalated")
...
document.getElementById("supDashCriticalRows").innerHTML = supDashCritical.length
  ? supDashCritical
      .map(
        (t) => `
      <tr>
        <td><a class="bo-row-link" href="ticket-detail.html?ticket=${encodeURIComponent(t.ticketNo)}&source=${t.source.toLowerCase()}">${t.ticketNo}</a></td>
        <td>${t.raisedBy}</td>
        <td>${t.organization}</td>
        <td>${t.issueType}</td>
        <td><span class="bo-pill bo-pill-tier">${t.tier}</span></td>
        <td><span class="bo-pill ${severityPillClass[t.severity] || ""}">${t.severity}</span></td>
...
/* ---------------- Tickets by Severity ---------------- */
document.getElementById("supDashSeverityChips").innerHTML = SEVERITIES.map((sev) => {
  const count = supDashTickets.filter((t) => t.severity === sev).length;
  return `
    <div class="bo-chip-row">
      <span class="bo-chip-left"><span class="bo-pill ${severityPillClass[sev] || ""}">${sev}</span></span>
...
document.getElementById("supDashIssueChips").innerHTML = CATEGORIES.map((category) => {
  const count = supDashTickets.filter((t) => (t.category || ISSUE_TYPE_CATEGORY[t.issueType]) === category).length;
  return `
    <div class="bo-chip-row">
