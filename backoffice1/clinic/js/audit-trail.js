/* Backoffice-safe fork of ../../js/audit-trail.js. Why forked: the root file
   wires every filter bar control (checkbox filters, date filters, search) to
   re-render immediately on every change ("live apply"). The Backoffice
   clinic pages instead gate all filter bars behind an Apply button -- each
   field just updates state + its own label on change, and the table only
   re-renders when Apply is clicked. Clear all filters keeps its existing
   immediate-apply behavior. Filtering, pagination, export, and the
   auditTrail data itself are otherwise identical to the root file. */
...
/* Apply-gated: picking/clearing a date just updates state + its own label;
   re-render waits for Apply (auditApplyBtn, below). */
