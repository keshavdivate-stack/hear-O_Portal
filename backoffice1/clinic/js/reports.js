/* Backoffice-safe fork of ../../js/reports.js. Why forked instead of shared:
   the root file's REPORT_PATIENTS array embeds real patient names + MRNs
   directly in the Patient Reports table's data/render logic. Only that:
   the `name` and `mrn` fields are dropped from REPORT_PATIENTS (the existing
   `username` field, e.g. "ABC-1254", already matches js/patient-list.js's
   Patient IDs and is now the sole patient identifier shown/searched/
   exported); the Patient table's row markup, search filter, and CSV export
   columns are adjusted to match. The Staffing and Program Outcomes report
   tabs are untouched -- they only ever showed aggregates derived from
   REPORT_PATIENTS (doctor names -- staff, not patient PHI -- counts,
   averages, org codes), never a patient name. */
...
const REPORT_PATIENTS = [
  { username: "ABC-1254", status: "priority", flag: true, since: "Since: 2d | 01.08.2028", monitoring: "monitored" },
  { username: "ABC-1252", status: "priority", flag: false, since: "Since: 2d | 01.08.2028", monitoring: "monitored" },
  { username: "ABC-1251", status: "priority", flag: false, since: "Since: 2d | 01.08.2028", monitoring: "unmonitored", monSince: "Since: 1d | 01.09.2028" },
  { username: "ABC-1238", status: "priority", flag: false, since: "Since: 3d | 01.07.2028", monitoring: "monitored" },
  { username: "ABC-1242", status: "priority", flag: false, since: "Since: 4d | 01.06.2028", monitoring: "monitored" },
  { username: "ABC-1283", status: "priority", flag: false, since: "Since: 8d | 01.02.2028", monitoring: "monitored" },
  { username: "ABC-1222", status: "active", since: "Since: 3d | 01.07.2028", monitoring: "monitored" },
  { username: "ABC-1222", status: "active", since: "Since: 3d | 01.07.2028", monitoring: "monitored" },
  { username: "ABC-1222", status: "active", since: "Since: 3d | 01.07.2028", monitoring: "monitored" },
  { username: "ABC-1220", status: "registered", since: "Since: 3d | 01.07.2028", monitoring: "none" },
  { username: "ABC-1222", status: "baseline", since: "Since: 3d | 01.07.2028", monitoring: "monitored", monInfo: true },
  { username: "ABC-1221", status: "none", monitoring: "unmonitored", monSince: "Since: 5d | 01.05.2028" },
];
...
function renderPatientRow(p) {
  return `
    <tr>
      ${checkboxCell(p.id)}
      <td><a class="lt-name ${p.status === "priority" ? "priority" : "active-name"}" href="patient-data.html">${p.username}</a></td>
      <td>${p.account}</td>
...
patientReportRowMenu.addEventListener("click", (e) => {
  ...
  if (item.dataset.action === "view") {
    ...
    openExportModal({
      ...
      filtersLabel: `Patient = ${patient.username}`,
      count: 1,
      countLabel: "Patients",
      rows: [patient],
