/* Backoffice-safe fork of ../../js/support-data.js. Why forked instead of
   shared: ticketList embeds a real patient name (patientName) on every
   Patient-type ticket. That field is dropped entirely below -- `who` is
   already an anonymous per-org patient identifier (e.g. "121-2010") for
   Patient-type tickets, so it alone is enough to identify "which patient"
   without naming them; ticket descriptions are technical/operational
   (voice engine, scheduling, compliance-score, sensor-sync issues) and name
   no clinical specifics, so they're unchanged. Everything else (categories,
   severities, states, support-team roster) is staff/config data, not patient
   PHI, and is unchanged from the root file. */
...
const ticketList = [
  ...
  { ticketId: "TCK-1231", organization: "B03", type: "Patient", who: "B03-5131", category: "Patient (Mobile/Web)", issueType: issueType("Patient (Mobile/Web)", 0), origin: "System Generated", severity: "Critical", state: "In Progress", scope: "Patient", assignedTo: "Daniel Roy", created: "18.08.2028", description: "Patient app remained paused beyond the expected threshold, blocking new recordings." },
  ...
  { ticketId: "TCK-1123", organization: "ATP", type: "Patient", who: "ATP-5023", category: "System Schedule Engine", issueType: issueType("System Schedule Engine", 0), origin: "User Created", severity: "Medium", state: "In Progress", scope: "Organization", assignedTo: "Tomer Levi", created: "14.08.2028", description: "Scheduled run missing a start date, escalated after repeated occurrence for this organization." },
  { ticketId: "TCK-1560", organization: "104", type: "Patient", who: "104-2290", category: "Voice Engine", issueType: issueType("Voice Engine", 1), origin: "System Generated", severity: "High", state: "Open", scope: "Patient", assignedTo: "Sarah Cline", created: "21.08.2028", description: "Recording uploaded but failed to process through the voice engine pipeline." },
  { ticketId: "TCK-1588", organization: "B01", type: "Patient", who: "B01-3390", category: "Sensors", issueType: issueType("Sensors", 1), origin: "System Generated", severity: "Low", state: "Resolved", scope: "Patient", assignedTo: "Maya Cohen", created: "09.08.2028", description: "Connected device failed to sync after a firmware update; resolved by re-pairing the device." },
];
